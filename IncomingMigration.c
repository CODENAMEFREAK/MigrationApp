...
#include "utilheaders/FreakUtil.h"
	...
	if(remoteNodeA==NULL || remoteNodeB==NULL)
       ...
       virConnectPtr nodeToMigrate = NULL;
		 ...
		 if(numOfDomainsOnA==1)
		 {
             ...
             destinationVMTo = getRemoteHostName( remoteNodeB);
		 }
		else{
			//i.e. the VM will be migrated From A to B . So incoming to A.
            ...
            free(domains);
            incomingVMFrom = getRemoteHostName(remoteNodeB);
            destinationVMTo = getRemoteHostName( remoteNodeA);  
		}
		
		int * jobType = (int *)malloc(sizeof(int));
		virTypedParameterPtr *  paramPtr= (virTypedParameterPtr *)malloc(sizeof(virTypedParameterPtr));
		int * numparams = (int *)malloc(sizeof(int));
		if(virDomainGetJobStats(domainToMigrate, jobType,paramPtr,numparams,1)==0)
		{
			printf("Got information");
			unsigned long long value;
			virTypedParamsGetULLong(*paramPtr,numparams,VIR_DOMAIN_JOB_DOWNTIME,&value);
		}
		else{
			printf("Couldn't fetch anything");
		}
